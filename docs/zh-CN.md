...
- **每个代理都在看板上。** 任务代理不能自己再派生子代理（`maxSubagentDepth`，默认 2）。没有这道限制时，代理可以派生调度器既看不见、也无法计量的助手——不计入全局上限、不受看门狗跟踪、不出现在看板上，却同样占用宿主的堆内存。实测：某个任务在后台派生了 12 个这样的隐藏助手，而看板上始终只显示一个任务。
- **正确的模型配正确的思考等级。** 如果某个角色钉的思考等级不被其回退模型支持，调度器会在派发前就撤回该候选的等级锁定——防止 UNSUPPORTED_REASONING_EFFORT 崩溃在一次性运行中杀死全部 6 个任务。
...
- **Roster** —— 分工表编辑器：按角色选模型（实时目录、按 provider 分组）、回退链排序、思考等级阶梯、角色并发上限、工具过滤、人设、自定义角色，还有一个"锁定分工表"的覆盖开关。模型下拉只列出你在 DSH 中已配置（提供 API key）的 provider，并自动跟随变化——需要 DSH 0.1.6+。
...
- id: swarm
  ...
  config:
    ...
    maxConcurrent: 10           # 同时运行的任务代理数（每次运行）
    maxTotalConcurrentAgents: 20 # 所有运行的总代理数上限（共享预算）
    ...
    spawnTimeoutSeconds: 3600   # 单任务运行的硬上限（0 = 关闭）
    maxSubagentDepth: 2         # 任务代理的再委派深度上限（1 = 不允许孙代理）
...
- **证据合约对文件是提示性的，对命令才是硬性的。** `evidence.files` 缺失或为空只会变成看板警告，不会因此关闭任务；只有 `evidence.commands` 失败才会阻止完成。因此范围控制是完成时审计，而不是写入拦截——没有任何机制能阻止代理写出 `writes` 范围之外的文件；调度器只会在派发时对重叠范围发出警告。
- **状态是文件级持久化，且仅在单个 DSH 进程内串行化。** 多个进程同时编辑同一工作区的 swarm 状态不受协调。请保证每个工作区只有一个宿主。
- **尝试身份尚未强制（J19）。** 每个任务会发布 `attempts` 计数，但没有逐次尝试的身份标识。若某次尝试已被重试取代，它迟到的结果目前无法与当前尝试的结果区分；结算路径只能依据任务*状态*，而无法依据结果*由谁产生*。`tests/fault-matrix.test.ts` 已记录该缺口（FM1/FM4/FM9），一旦加入围栏会自动收紧断言。实际影响：如果你在上一个子代理仍在运行时重试任务，较旧子代理的结果仍可能被记录。
...
v0.6.14，日常使用中。测试覆盖调度器对假 spawn provider 的端到端行为（**150 个用例**：派发、放行、架构师注入、评审循环、人工评审门、模型回退轮换、熔断、重试退避、配额暂停/恢复、救援路径、证据合约、写入范围警告、事件日志合法性、再委派深度、工具过滤、工作区作用域、通知容错、尝试记账、界面本地化、努力档位阶梯与同模型降档重试、看门狗存活判定与回收采纳、客户端插件加固、客户端连接预算、针对生产事件存储的实时启动审计，以及针对调度器不变量的 **10 个对抗性故障矩阵用例**），并已在真实部署与隔离的端到端冒烟运行上完成验证。
