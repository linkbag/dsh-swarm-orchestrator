...
### 可靠性说明（v0.5.8 – v0.6.18）

以下问题来自对 47 次运行、184 次任务失败及后续数次事故的诊断，均已修复并补上回归测试：
...
- **被采纳的报告还必须由"当前这一次尝试"写成。** `.dsh-swarm/task-<id>.json` 按任务 id 命名而非按运行隔离，因此复用同名任务会共用同一个文件。现在若报告的写入时间早于当前尝试的 `task/started`，它会被忽略并记入日志，而不会被当作本次尝试的成果。
- **恢复仅限运行中的运行。** 孤儿恢复只会重排"所属运行仍在运行"的任务，因此终结状态的运行其任务保持冻结，不会在每次宿主重启时被反复失败。
- **评审结论从完整回复中解析。** 旧解析器只读 2000 字摘要，评审越认真越容易被误判为未给出结论。现在漏写结论行的评审会先收到一次明确的重问，之后才判失败。
- **看门狗回收会采纳已完成的成果并启动重试退避。** 被回收前一刻刚写完报告的子代理会被记为完成而不是失败——旧路径曾让一个运行在 `retrying` 里困了七个小时。孤儿恢复会重新排起滞留的 `retrying` 任务，每次新尝试的静默时钟也从零起算。
- **存活判定看进展，不看耗时。** 派发超时改为随每次心跳重置的滑动无进展窗口，名册里可按角色覆盖 `spawnTimeoutSeconds`——健康的子代理绝不会因为跑得久而被动杀。
- **Web 客户端只持有一条 `/swarm/events` 连接**（引用计数，由标签页、徽标、头部弹窗与所有派发卡片共享）。此前每个渲染出的派发卡片各开一条流，耗尽浏览器每源约 6 条连接的预算，宿主明明健康、整个界面却冻结。
- **仅证据失败不再重跑工作。** 先做一次只重跑命令的复核，仍失败就把任务停给人工（`blocked` + 人工评审）。记录携带退出码、超时状态、耗时与输出尾部——"摘要全绿但退出码非零"的测试从事件日志就能诊断。
...
- **Roster** —— 分工表编辑器：按角色选模型（实时目录、按 provider 分组）、回退链排序、思考等级阶梯、角色并发上限、逐角色派发超时、工具过滤、人设、自定义角色，还有一个"锁定分工表"的覆盖开关。模型下拉只列出你在 DSH 中已配置（提供 API key）的 provider，并自动跟随变化——需要 DSH 0.1.6+。
...
- **语言** —— Swarm 界面自动跟随 DSH 的语言偏好（设置 → 通用 → 语言）：内置英文与简体中文，切换后无需刷新即可生效。

## 一次运行的生命周期

1. **派发** —— 用自然语言告诉 Agent 你要什么，它调 `swarm_dispatch` 提交任务图。新运行先停在 *planning*，一个代理都不会启动。
2. **执行** —— 调度器通过服务自有的锚定代理派发任务代理：就算派发它的那个聊天早就关了，运行照样继续。
3. **评审** —— 带评审者的任务先被裁决；驳回即带反馈重新入队。带证据合约的任务必须交出它承诺的文件和通过的命令。
4. **报告** —— 运行收尾时给出报告：谁做了什么、用的什么模型、回退/重试/评审统计。全程是可回放的 JSONL 追加日志。
...
- **回退链** —— 主模型不可用时按序尝试。
- **思考等级 + 等级阶梯** —— 该角色锁定的推理力度；档位随尝试推进、最终必然落到"不锁定"，模型不支持的档位会在派发前就被撤回。
- **并发上限** —— 限制该角色同时运行的代理数。
- **派发超时** —— 按角色覆盖"静默子代理被回收"的无进展窗口。
...
任务还支持**证据合约**——`evidence: { files: [...], commands: [...] }`，机器校验通过任务才算关闭；**写入范围**——`writes: [files]`，并发建造者互不越界（调度器会对重叠范围给出警告）；以及**人工评审门**，裁决权交还看板。证据命令失败不再重跑已完成的工作：先做一次只重跑命令的复核，若仍失败——退出码非零或超时（`evidenceTimeoutMs`，默认 120 秒）——任务会停在看板上等人工裁决，退出码与输出尾部会记入事件日志。
...
- id: swarm
  ...
  config:
    ...
    evidenceTimeoutMs: 120000   # 每条证据命令的超时（1 秒 – 10 分钟）
    ...
    spawnTimeoutSeconds: 3600   # 每个子代理的滑动无进展窗口，随心跳重置（0 = 关闭）
    maxSubagentDepth: 1         # 任务代理的再委派深度上限（1 = 不允许再委派）
    bootGraceSeconds: 3         # 插件加载后等待孤儿恢复的时间
```
...
| Spawn timeout (sec) | 3600 | 每个子代理的滑动无进展窗口——每次心跳重新计时，干活的子代理不会因耗时被杀；从未启动或静默的才会被回收（0 = 关闭） |
...
- **浏览器半区**：Swarm 标签页、聊天进度卡片、头部弹窗、设置页分区——全部由一条引用计数的 SSE 连接推送看板快照；模型选择器与"模型设置"页使用同一套 LLM RPC。
- **逐角色思考等级**通过 DSH 的 `agent/request` waterfall 按请求注入，只作用于受追踪的蜂群子代理——派发前会对照部署声明的模型能力校验（派发与评审锁定一视同仁），并记录在 `task/started` 上。
- **确定性回放**：状态是事件日志的折叠，并带合法性守卫——再刁钻的事件序列也无法让已中止的运行复活、让任务被重复完成。

## 已知限制

这里直白写明，因为在生产环境里踩到一条限制的代价，远高于事先读到它。

- **看板呈现的是磁盘真相，而磁盘可能落后于模型。** 任务代理可能在完成工作后并没有走完协议要求的全部记账步骤（例如写出 `.dsh-swarm/task-<id>.json` 报告）。看板显示的是真实记录下来的内容，而不是代理自称做了什么。请把任务摘要当作“记录了什么”的证据，交付物仍需自行核验。
- **证据合约对文件是提示性的，对命令才是硬性的。** `evidence.files` 缺失或为空只会变成看板警告，不会因此关闭任务；只有 `evidence.commands` 失败才会阻止完成（先做一次只重跑命令的复核，持续失败就把任务停给人工，而不是重跑工作）。因此范围控制是完成时审计，而不是写入拦截——没有任何机制能阻止代理写出 `writes` 范围之外的文件；调度器只会在派发时对重叠范围发出警告。
- **状态是文件级持久化，且仅在单个 DSH 进程内串行化。** 多个进程同时编辑同一工作区的 swarm 状态不受协调。请保证每个工作区只有一个宿主。
- **成员消息是单向的。** 任务代理可以向上汇报进度，但彼此之间不能通信；协作通过 DAG（依赖与写入范围）而非对话完成。
- **同一角色可以同时持有多个任务。** 调度按角色与全局上限约束并发，而不是“每个代理同时只开一个任务”。
- **恢复是“每次启动有界”，不是“全局有界”。** 孤儿恢复在每次宿主启动时最多重排一次滞留任务。反复重启的宿主会反复重排同一个不愿推进的任务——这是有意为之，以免未完成的任务被悄悄放弃。
- **看板标题中的版本号是启动时加载的版本**，而不是磁盘上的版本。手工编辑 `duty-table.json` 同样需要重启宿主；通过名册 UI 所做的修改则即时生效。
- **`lib/` 是构建产物，不入库。** 对 `client/` 的源码改动在运行 `npm run build` 之前不会生效；`tests/bundle-freshness.test.ts` 会在构建产物过期时报错。

## 状态

v0.6.18，日常使用中。测试覆盖调度器对假 spawn provider 的端到端行为（**173 个用例**：派发、放行、架构师注入、评审循环、人工评审门、模型回退轮换、熔断、重试退避、配额暂停/恢复、救援路径、证据合约与仅证据复核门、写入范围警告、事件日志合法性、再委派深度、工具过滤、工作区作用域、通知容错、尝试记账、界面本地化、思考等级阶梯与同模型降档重试、部署声明的思考等级预检（派发与评审锁定）、看门狗存活判定与回收采纳、客户端插件加固、客户端连接预算、针对生产事件存储的实时启动审计，以及针对调度器不变量的 **10 个对抗性故障矩阵用例**），并已在真实部署与隔离的端到端冒烟运行上完成验证。
