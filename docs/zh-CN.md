...
v0.6.19，日常使用中。测试覆盖调度器对假 spawn provider 的端到端行为（**187 个用例**：派发、放行、架构师注入、评审循环、人工评审门、模型回退轮换、熔断、重试退避、配额暂停/恢复、救援路径、证据合约与仅证据复核门、写入范围警告、事件日志合法性、再委派深度、工具过滤、工作区作用域、通知容错、尝试记账、界面本地化、思考等级阶梯与同模型降档重试、部署声明的思考等级预检（派发与评审锁定）、看门狗存活判定与回收采纳、客户端插件加固、客户端连接预算、针对生产事件存储的实时启动审计，以及针对调度器不变量的 **10 个对抗性故障矩阵用例**），并已在真实部署与隔离的端到端冒烟运行上完成验证。
