...
**dsh-swarm-orchestrator** 在 DeepSeek Harness 内把一个目标变成一支有监督的代理团队：架构师把你的方案审阅精炼成 `PLAN.md`，多个建造者按任务 DAG 并行执行，评审代理把关质量，集成代理收尾交付。每次运行都先经你放行、每个角色都钉选你实时目录里的模型、每项交付都可机器校验，全程在实时看板与流程图上可见。
...
```text
   ...
   你的聊天 Agent            （自行规划/调研完全没问题——
         ...
         ▼
  ▣ 放行门 ENDORSEMENT        运行先停在 *planning*，一个代理都不启动，
         │                     直到你在 Swarm 标签页点 Endorse
         ▼
  ┌─────────────────┐
...
- 新运行停在 **planning**——打开 **Swarm** 标签页点 **Endorse**。放行前一个代理都不会启动。
